...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defvar mode-line-bar-i         (! (pl/percent-xpm mode-line-height 100 0 100 0 3 "#a6e22e" nil)))

(defvar mode-line-bar-n          (! (pl/percent-xpm mode-line-height 100 0 100 0 3 "#5fd7ff" nil)))

(defvar mode-line-bar-v          (! (pl/percent-xpm mode-line-height 100 0 100 0 3 "#fd971f" nil)))


(defun doom-mode-line (&optional id)
  `(:eval
    (let* ((active (eq (selected-window) mode-line-selected-window))
           (lhs (list (propertize " " 'display ((cond ((evil-motion-state-p) mode-line-bar)
                                                      ((evil-visual-state-p) mode-line-bar-v) 
                                                ((evil-emacs-state-p) mode-line-bar-i) 
                                  ((evil-normal-state-p)  mode-line-bar-n)
                                  (t  mode-line-bar-i))
                                                )))
                      (*flycheck)
                      (*macro-recording)
